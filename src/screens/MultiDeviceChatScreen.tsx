...
interface MultiDeviceChatScreenProps {
  route: { params: { restaurantName: string; groupId: string } };
  navigation: any;
}

export const MultiDeviceChatScreen: React.FC<MultiDeviceChatScreenProps> = ({ route, navigation }) => {
  ...
  const chatRoom = multiDeviceChatService.getChatRooms().find(room => room.id === chatRoomId);
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      multiDeviceChatService.unsubscribe('typing', handleTyping);
      multiDeviceChatService.unsubscribe('messageDeleted', handleMessageDeleted);
      multiDeviceChatService.unsubscribe('messageStatusUpdate', handleMessageStatusUpdate);
      multiDeviceChatService.unsubscribe('reactionAdded', handleReactionAdded);
      multiDeviceChatService.unsubscribe('usersUpdated', handleUsersUpdated);
    };
  }, [chatRoomId]);
  ...
  const setupEventListeners = () => {
    ...
    multiDeviceChatService.subscribe('typing', handleTyping);
    multiDeviceChatService.subscribe('messageDeleted', handleMessageDeleted);
    multiDeviceChatService.subscribe('messageStatusUpdate', handleMessageStatusUpdate);
    multiDeviceChatService.subscribe('reactionAdded', handleReactionAdded);
    multiDeviceChatService.subscribe('usersUpdated', handleUsersUpdated);

    // Mark messages as read
    multiDeviceChatService.markAsRead(chatRoomId);
  };
  ...
  const loadMessages = () => {
    const roomMessages = multiDeviceChatService.getMessages(chatRoomId);
  ...
  const sendMessage = async () => {
    if (!newMessage.trim() || !currentDevice) return;

    try {
      await multiDeviceChatService.sendMessage(chatRoomId, newMessage.trim(), replyTo || undefined);
      setNewMessage('');
      setReplyTo(null);
      setIsTyping(false);
      multiDeviceChatService.setTyping(chatRoomId, currentDevice.deviceId, false);
    } catch (error) {
  ...
  const handleTextInputChange = (text: string) => {
    ...
    if (text.length > 0 && !isTyping) {
      setIsTyping(true);
      multiDeviceChatService.setTyping(chatRoomId, currentDevice.deviceId, true);
    ...
    Alert.alert(
      ...
      [
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          style: 'destructive',
          onPress: async () => {
            const success = await multiDeviceChatService.deleteMessage(chatRoomId, messageId);
            if (!success) {
  ...
  const addReaction = async (messageId: string, emoji: string) => {
    await multiDeviceChatService.addReaction(chatRoomId, messageId, emoji);
  ...
  const renderMessage = ({ item, index }: { item: ChatMessage; index: number }) => {
    ...
    const isCurrentUser = item.senderId === currentDevice.deviceId;
    ...
    if (isCurrentUser) {
      senderName = currentDevice.displayName;
    } else if (item.senderId.startsWith('mock-')) {
      senderName = item.senderId.replace('mock-', '').replace(/\b\w/g, l => l.toUpperCase());
    } else {
      // Try to find user in chat room participants
      const participant = chatRoom?.participants.find(p => p.id === item.senderId);
      senderName = participant?.fullName || 'Unknown';
    }
      ...
      <View>
        ...
        <View style={[
          ...
          {showAvatar && !isCurrentUser && (
            <Image 
              source={{ uri: chatRoom?.participants.find(p => p.id === item.senderId)?.profilePicture || 'https://picsum.photos/200/200?random=999' }} 
              style={styles.avatar} 
            />
          )}
          ...
          <View style={styles.messageActions}>
            ...
            <TouchableOpacity
              style={styles.actionButton}
              onPress={() => setReplyTo({ messageId: item.id, text: item.text, senderName })}
            >
  ...
  const renderTypingIndicator = () => {
    if (typingUsers.length === 0 || !currentDevice) return null;

    const typingNames = typingUsers
      .map(userId => {
        if (userId === currentDevice.deviceId) {
